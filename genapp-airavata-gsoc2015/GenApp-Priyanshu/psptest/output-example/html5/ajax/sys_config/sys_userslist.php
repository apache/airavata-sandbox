...
function get_userinfo( $error_json_exit = false ) {
   ...
   $runs = $use_db->psptest->running->find();
   foreach ( $runs as $v ) {
       $uuid = $v['_id'];
       $job = $use_db->psptest->jobs->findOne( array( "_id" => $uuid ), array( "user" => 1 ) );
       $name = $job[ 'user' ];
   ...
   $users = $use_db->psptest->users->find();
   ...
   foreach ( $users as $v ) {
       ...
       $userinfo[] = 
           array( 
               ...
               ,"jobs-not-removed"   => $use_db->psptest->jobs->count( array( "user" => $name ) )
