...
function jobcancel( $jobs,  $error_json_exit = false ) {
    ...
    $runs = $use_db->psptest->running->find( array( "_id" => array( '$in' => $jobs ) ) );
    ...
    foreach ( $runs as $v ) {
       $uuid = $v['_id'];
       $job = $use_db->psptest->jobs->findOne( array( "_id" => $uuid ) );
       $pids = $v['pid'];
    ...
    foreach ( $tokill as $k => $v ) {
        if ( !isset( $appconfig[ 'resources' ][ $k ] ) ) {
            ...
            $kill = $appconfig[ 'resources' ][ $k ] . " /opt/lampp/htdocs/psptest/util/ga_killprocs.pl /opt/lampp/htdocs/psptest/log $k all " . implode( ' ', $v );
    ...
    foreach ( $tokillparent as $k => $v ) {
        if ( !isset( $appconfig[ 'resources' ][ $k ] ) ) {
            ...
            $kill = $appconfig[ 'resources' ][ $k ] . " /opt/lampp/htdocs/psptest/util/ga_killprocs.pl /opt/lampp/htdocs/psptest/log $k child " . implode( ' ', $v );
